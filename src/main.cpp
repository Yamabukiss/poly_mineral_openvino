...
void PicoDet::drawBboxes(const cv::Mat &bgr, const std::vector<BoxInfo> &bboxes) {
    ...
    if (!bboxes.empty())
    {
        static std::vector<std::vector<cv::Point2f>> last_frame_points_saver;
        ...
        for (size_t i = 0; i < bboxes.size(); i++) {
            ...
            std::vector<cv::Point2f> added_weights_points=pointAssignment(points_vec,last_frame_points_saver);
            last_frame_points_vec_.emplace_back(added_weights_points);
            getPnP(added_weights_points,bbox.label);
            ...
            cv::putText(image,label_array[bbox.label],cv::Point2f(std::max(float(0),added_weights_points[1].x-10),std::max(added_weights_points[1].y-10,float(0))),cv::FONT_HERSHEY_SCRIPT_SIMPLEX,2,color,3);
    }
